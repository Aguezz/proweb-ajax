...
$dataMahasiswa = $db->Execute("SELECT * FROM `mahasiswa` ORDER BY `nim` ASC");
    ...
    <tbody>
        <?php while ($mahasiswa = $dataMahasiswa->FetchNextObj()) : ?>
            <tr>
                ...
                <td>
                    <a href="" onclick="event.preventDefault(); handleEditButtonClick('<?= $mahasiswa->nim ?>')">Edit</a>
                    <a href="" onclick="event.preventDefault(); handleDeleteButtonClick('<?= $mahasiswa->nim ?>')">Delete</a>
                </td>
            </tr>
        <?php endwhile ?>
    </tbody>
